   ...
   function verify_session_time_out()
    {
        ...
        if($user_id and !empty($user_id) and isset($user_id))
        {
            ...
            if($interval > $session_timeout)
            {
				...
				$update_login=$login_table->update_login_user($user_id);
                unset_user_session();
                return false;
            }
        }
	...
	function send_admin_confirmation_email($user_id,$first_name,$last_name)
	{
			$user_table = new user_table();
	        $user_data = $user_table->retrieve_user($user_id);
	        if($user_data)
	        {
	            ...
	            $email = new user_mail();
	            $email->set_to($user_email);
	            $email->set_subject('Account Admin Approval');
	            $message_body = $email->generate_admin_confirmation_email_body($first_name,$last_name);
	            $email->set_message_body($message_body);
	            $return = $email->send_mail();
	            if($return)
	            {
